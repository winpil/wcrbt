...
<html>
	...
	<body>
    	<ul class="nav nav-tabs sys-font">
			<li class="active"><a href="${"$"}{ctx }/sys/${table.lowerName}WebController/list">${table.tableNameCH}列表</a></li>
			<li><a href="${"$"}{ctx }/sys/${table.lowerName}WebController/form">${table.tableNameCH}添加</a></li>
		</ul>
		<form:form id="searchForm" modelAttribute="${table.lowerName}" action="${"$"}{ctx }/sys/${table.lowerName}WebController/list" method="post" class="breadcrumb form-search">
			<ul class="ul-form">
		...
		<c:forEach items="${"$"}{page.list}" var="entity">
			<tr>
				...
				<td>
    				<a href="${"$"}{ctx }/sys/${table.lowerName}WebController/form?id=${"$"}{entity.id }">修改</a>
					<a href="${"$"}{ctx }/sys/${table.lowerName}WebController/delete?id=${"$"}{entity.id }" onclick="return confirm('您确定要删除该项吗？')">删除</a>
				</td>
			</tr>
		</c:forEach>
		</tbody>
