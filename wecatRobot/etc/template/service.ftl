...
import com.java.controller.response.Return${table.shortName};
...
public class ${table.className}Service extends BaseService<${table.className}Dao, ${table.className}> {
	...
	public Return${table.shortName} getReturn${table.shortName}(${table.className} entity,HttpServletRequest request){
	...
	public List<Return${table.shortName}> getReturn${table.shortName}List(List<${table.className}> entityList,HttpServletRequest request){
		List<Return${table.shortName}> list = new ArrayList<Return${table.shortName}>();
		if(entityList != null && entityList.size()>0){
			for(${table.className} entity : entityList){
				list.add(getReturn${table.shortName}(entity, request));
			}
		}
		return list;
	}
