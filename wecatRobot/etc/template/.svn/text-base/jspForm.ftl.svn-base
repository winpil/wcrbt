...
<html>
  	...
  	<body>
  		<ul class="nav nav-tabs sys-font">
			<li><a href="${"$"}{ctx }/sys/${table.lowerName}WebController/list">${table.tableNameCH}列表</a></li>
			<li class="active"><a href="${"$"}{ctx }/sys/${table.lowerName}WebController/form">${table.tableNameCH}添加</a></li>
		</ul>
		<sys:message content="${"$"}{message }"/>
  		<form:form id="inputForm" modelAttribute="entity" action="/${"$"}{projectName}/sys/${table.lowerName}WebController/save" method="post" class="form-horizontal">
			<form:hidden path="id"/>
